...
public:
	OffboardControl() : Node("offboard_control")
	{
		...
		"/joy", 10, [this](const sensor_msgs::msg::Joy::SharedPtr msg) {
			// Exemplo: Botão 0 (geralmente o 'X' no PS4) ativa o Offboard
			if (msg->buttons[0] == 1) {
			this->publish_vehicle_command(VehicleCommand::VEHICLE_CMD_DO_SET_MODE, 1, 6);
			this->arm();
			mission_finished_ = false; 
			RCLCPP_INFO(this->get_logger(), "Modo Autônomo Ativado pelo Controle PS4");
			}
			
			// Exemplo: Usar o analógico para interferir no Yaw ou posição
			// msg->axes[0] seria o analógico esquerdo
		});
	}
