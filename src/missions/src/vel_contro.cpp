...
OffboardControl::OffboardControl() : Node("minimal_publisher") {
	...
	//service for computional vision
    client_ = this->create_client<px4_msgs::srv::DetectAruco>("detect_aruco");
...
std::string stateToString(State state) {
    switch(state) {
        ...
        case State::OFFBOARD:
            return "OFFBOARD";
	case State::SEARCH_GABARITO:
            return "SEARCH_GABARITO";
        default:
            return "UNKNOWN";
    }
}
void OffboardControl::arm_timer_callback() {
    switch(current_state) {
	    ...
	    case State::SEARCH_GABARITO:
            if(!flightCheck || arm_state == px4_msgs::msg::VehicleStatus::ARMING_STATE_DISARMED || failsafe == true) {
                current_state = State::SEARCH_BASE;
                RCLCPP_INFO(this->get_logger(), "identifed target");
            }
            state_search_gabarito();
            break;

	     case State::SEARCH_BASE:
            if(!flightCheck || arm_state == px4_msgs::msg::VehicleStatus::ARMING_STATE_DISARMED || failsafe == true) {
...
void OffboardControl::state_takeoff() {
    myCnt = 0;
    publish_vehicle_command(px4_msgs::msg::VehicleCommand::VEHICLE_CMD_NAV_TAKEOFF, 1.0, 0.0, 1.7); // param7 is altitude in meters
...
//service client callback gabarito
void OffboardControl::state_search_gabarito() {
    myCnt = 0;
    // 1. Define uma velocidade de busca (ex: andar em círculos ou linha reta)
    velocity.x = 0.2; // Move para frente devagar
    velocity.y = 0.0;
    velocity.z = 0.0;
    RCLCPP_INFO(this->get_logger(), "Searching for Gabarito");
	 while (!client_->wait_for_service(1s))
        {
            RCLCPP_WARN(this->get_logger(), "Waiting for the server...");
        }

        auto request = std::make_shared<px4_msgs::srv::DetectAruco::Request>();
        request->start = true;

        client_->async_send_request(
            request, std::bind(&OffboardControl::callbackCallbackGabarito, this, _1, _2));
...
void OffboardControl::take_off() {
    publish_vehicle_command(px4_msgs::msg::VehicleCommand::VEHICLE_CMD_NAV_TAKEOFF, 0.0, 0.0, 5.0); // param7 is altitude in meters
    RCLCPP_INFO(this->get_logger(), "Takeoff command send");
}
...
void OffboardControl::callbackCallbackGabarito(rclcpp::Client<px4_msgs::srv::DetectAruco>::SharedFuture future, const geometry_msgs::msg::Twist::SharedPtr msg)
{

    // 1. Obtém o resultado do "futuro"
    auto response = future.get();

    if (response->target_detected) {
        RCLCPP_INFO(this->get_logger(), "Gabarito achado! ID: %ld", response->target_number);
        
        // 2. Armazena os dados que o gabarito forneceu
        this->target_id = response->target_number;
        this->target_shape = response->target_poligon;
        
        // 3. Lógica de Divisibilidade (Exemplo SAE)
        // Aqui você já pode definir qual será a base de pouso
        if (this->target_id % 3 == 0) this->base_alvo = 3;
        else if (this->target_id % 4 == 0) this->base_alvo = 4;
        else if (this->target_id % 5 == 0) this->base_alvo = 5;

        RCLCPP_INFO(this->get_logger(), "Calculado: Alvo é a Base %d", this->base_alvo);
	...
	// Se não detectou nada neste frame, não faz nada. 
	// O seu 'arm_timer_callback' vai chamar o serviço de novo no próximo ciclo.
	RCLCPP_DEBUG(this->get_logger(), "Gabarito não visível neste frame.");
    }
}
